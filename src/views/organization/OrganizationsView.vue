<template>
  <div class="container mx-auto xl:max-w-6xl pt-16 px-4 mt-8" ref="main">
    <div class="text-4xl font-bold">Organisasi</div>

    <div class="flex flex-row justify-between">
      <div class="mt-8 text-secondary lg:w-3/5">
        Telusuri dan temukan organisasi-organisasi di Open Data Gunungkidul yang memainkan peran kunci dalam pembangunan ekosistem data terbuka.
      </div>
    ...
    <div class="mt-4">
      ...
      <div v-else>
        ...
        <div v-else class="text-error font-bold">Organisasi tidak ditemukan</div>
      </div>
    </div>
  </div>
</template>
...
const reload = async () => {
  ...
  let response = (await ckanApi.getOrganizations(query.value, selectedSort.value)).data.result
  data.value = response
